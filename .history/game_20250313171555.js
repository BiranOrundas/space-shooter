...
function shootBullet() {
  const now = Date.now();
  if (now - player.lastShot > 10 ) { // Cooldown in ms
    bullets.push({
...
// Create enemy ships
function spawnEnemy() {
if (Math.random() < 100 && !gameOver) {
// Eğer boost aktifse, düşmanların hızını artır
const speedFactor = player.isBoosting ? 25.5 : 1; 
const enemySpeed = player.isBoosting ? Math.random() * speedFactor : Math.random() * 1 + 2;
...
function updateLifeBar() {
  ...
  if (player.life > 60) {
    ...
    lifeBar.style.backgroundColor = '#ff0';
  } else {
